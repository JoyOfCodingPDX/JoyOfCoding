...
public class GradeBookXmlTest {
  ...
  public void letterGradeRangesArePersistedToXml() throws IOException, TransformerException, ParserException {
    ...
    GradeBook.LetterGradeRanges undergradRanges = book.getLetterGradeRanges(Student.Section.UNDERGRADUATE);
    morphLetterGradeRanges(undergradRanges, 1);

    GradeBook.LetterGradeRanges gradRanges = book.getLetterGradeRanges(Student.Section.GRADUATE);
    morphLetterGradeRanges(gradRanges, 2);

    GradeBook book2 = writeAndReadGradeBookAsXml(book);

    GradeBook.LetterGradeRanges undergradRanges2 = book2.getLetterGradeRanges(Student.Section.UNDERGRADUATE);
    assertRangesAreEqual(undergradRanges, undergradRanges2);

    GradeBook.LetterGradeRanges gradRanges2 = book2.getLetterGradeRanges(Student.Section.GRADUATE);
    assertRangesAreEqual(gradRanges, gradRanges2);
  }

  private void assertRangesAreEqual(GradeBook.LetterGradeRanges ranges, GradeBook.LetterGradeRanges ranges2) {
    for (LetterGrade letterGrade : LetterGrade.values()) {
      ...
      GradeBook.LetterGradeRanges.LetterGradeRange range2 = ranges2.getRange(letterGrade);
      assertThat("Range for " + letterGrade, range2.minimum(), equalTo(range.minimum()));
    }
  }

  private void morphLetterGradeRanges(GradeBook.LetterGradeRanges ranges, int morphValue) {
    for (LetterGrade letterGrade : LetterGrade.values()) {
      GradeBook.LetterGradeRanges.LetterGradeRange range = ranges.getRange(letterGrade);
      if (range == null) {
        continue;
    ...
    GradeBook.LetterGradeRanges.LetterGradeRange fRange = ranges.getRange(LetterGrade.F);
...
//    System.out.println(new String(bytes));
  ...
  void androidProjectTypeIsPersistedToXml() throws ParserException, IOException, TransformerException {
    persistProjectOfType(Assignment.ProjectType.ANDROID);
  }
